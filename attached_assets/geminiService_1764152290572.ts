...
import { SITPattern, AnalysisResult, InnovationResult, TechnicalSpec, ThreeDSceneDescriptor, SceneObject, MeshType, MaterialType } from '../types';

declare global {
  interface Window {
    THREE: any; // Declare THREE globally if loaded via CDN
  }
}
...
async function runWithRetry<T>(fn: () => Promise<T>, retries = 5, delay = 4000): Promise<T> {
  try {
    ...
    // Robust check for rate limit errors (checking message string and error object properties)
...
export const analyzeProduct = async (input: string, image?: string): Promise<AnalysisResult> => {
  return runWithRetry(async () => {
    ...
    if (image) {
        // Image should be passed as base64 string (removing data:image/jpeg;base64, prefix if present)
    ...
    const schema: SchemaParams = {
     ...
     const response = await ai.models.generateContent({
      ...
      config: {
        ...
        responseSchema: schema,
        thinkingConfig: {
          thinkingBudget: 1024 // Reduced to save tokens
        }
      }
    });
...
export const generate3DScene = async (innovation: InnovationResult): Promise<ThreeDSceneDescriptor> => {
  return runWithRetry(async () => {
    ...
    const prompt = `
      ...
      {
        "id": "unique_string_id",
        "type": "mesh_type_enum", // 'box', 'sphere', 'cylinder', 'plane'
        "position": [x, y, z], // numbers, relative to scene center [0,0,0]
        "rotation": [x_radians, y_radians, z_radians], // numbers, rotation around axes in radians
        "scale": [sx, sy, sz], // numbers, size multipliers
        "color": "hex_string", // e.g., "#RRGGBB"
        "material": "material_type_enum", // 'standard' or 'wireframe' (prefer wireframe for schematic)
        "args": [arg1, arg2, ...] // numbers.
            // For 'box': [width, height, depth]
            // For 'sphere': [radius] (segments will be default)
            // For 'cylinder': [radiusTop, radiusBottom, height] (segments will be default)
            // For 'plane': [width, height]
      }
      ...
      Focus on representing the *essence* of the innovation. For example, if 'Subtraction' removed a screen, don't render a screen. If 'Task Unification' used ambient light, show a simple light source.

      Return only a valid JSON object of type \`ThreeDSceneDescriptor\`, containing an 'objects' array.
    `;

    // Simplified schema to strict JSON Schema standards supported by Gemini
    const sceneObjectSchema: SchemaParams = {
      type: Type.OBJECT,
      properties: {
        id: { type: Type.STRING },
    ...
    const response = await ai.models.generateContent({
      ...
      config: {
        ...
        responseSchema: schema,
        thinkingConfig: {
          thinkingBudget: 0 // Reduced to save tokens & speed up
        }
      }
    });
