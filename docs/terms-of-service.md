
# Terms of Service for ReversR

**Last Updated:** January 26, 2025

## Acceptance of Terms

By accessing or using ReversR ("the App"), you agree to be bound by these Terms of Service ("Terms"). If you do not agree to these Terms, do not use the App.

## Description of Service

ReversR is a mobile application that uses artificial intelligence to analyze product images and apply Systematic Inventive Thinking (SIT) methodologies to identify:
- Product components and their functions
- Product attributes (quantitative and qualitative)
- Neighborhood resources
- System boundaries
- Innovation opportunities

## User Responsibilities

You agree to:
- Provide accurate information when using the App
- Use the App only for lawful purposes
- Not upload inappropriate, offensive, or illegal content
- Not attempt to reverse engineer or hack the App
- Respect intellectual property rights of others

## Intellectual Property

### Your Content
- You retain ownership of images and descriptions you upload
- By using the App, you grant us a license to process your content for analysis purposes

### Our Content
- The App, including its design, code, and AI-generated analyses, is owned by ReversR
- You may not copy, modify, or distribute the App without permission

## AI-Generated Content

- Analyses are generated using AI and may contain errors or inaccuracies
- Results are provided "as is" for educational and ideation purposes
- We do not guarantee the accuracy, completeness, or usefulness of AI-generated content
- You are responsible for verifying any information before using it

## Disclaimer of Warranties

THE APP IS PROVIDED "AS IS" WITHOUT WARRANTIES OF ANY KIND, EXPRESS OR IMPLIED, INCLUDING BUT NOT LIMITED TO:
- Merchantability
- Fitness for a particular purpose
- Non-infringement
- Accuracy of AI-generated analyses

## Limitation of Liability

To the maximum extent permitted by law, ReversR shall not be liable for:
- Any indirect, incidental, or consequential damages
- Loss of data, profits, or business opportunities
- Damages resulting from use or inability to use the App
- Decisions made based on AI-generated analyses

## Third-Party Services

The App uses third-party services, including:
- Google Gemini API for AI processing
- Expo for app development and distribution

Your use of these services is subject to their respective terms and privacy policies.

## Privacy

Your use of the App is also governed by our Privacy Policy, which is incorporated into these Terms by reference.

## Termination

We reserve the right to:
- Suspend or terminate your access to the App at any time
- Modify or discontinue the App without notice

You may stop using the App at any time by uninstalling it from your device.

## Changes to Terms

We may update these Terms from time to time. Continued use of the App after changes constitutes acceptance of the updated Terms.

## Governing Law

These Terms are governed by the laws of [Your Jurisdiction], without regard to conflict of law principles.

## Dispute Resolution

Any disputes arising from these Terms or use of the App shall be resolved through:
1. Good faith negotiation
2. Binding arbitration (if applicable in your jurisdiction)

## Severability

If any provision of these Terms is found to be unenforceable, the remaining provisions shall remain in full effect.

## Contact Information

For questions about these Terms, please contact us at:
[Your Contact Email]

## Acknowledgment

By using ReversR, you acknowledge that you have read, understood, and agree to be bound by these Terms of Service.

---

**ReversR** - Systematic Inventive Thinking Made Accessible
